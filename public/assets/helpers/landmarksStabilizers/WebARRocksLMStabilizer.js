...
const WebARRocksLMStabilizer = (function () {
  ...
  function compute_sigmas(cursor, lastPositions, lastPredicted, sigmas) {
    ...
    for (let lmi = 0; lmi < n; ++lmi) {
      // loop over landmarks
      const sigma = sigmas[lmi];
      reset_vec2(sigma);

      for (let i = 0; i < m - 1; ++i) {
        // loop over records (dot not take account of current record)
  ...
  function update_predictionWithPhysics(
    predicteds,
    velocities,
    accelerations,
    dt
  ...
  const superThat = {
    instance: function (spec) {
      ...
      let _lastTimestamps = null,
        _lastPositionsPx = null;
      let _lmsPx = null;
      let _lastPredicteds = null;
      let _lmsMeanPx = null,
        _lmsSigmaPx = null;
      let _lmsVelocities = null,
        _lmsAccelerations = null;
      let _lmsPredictedPx = null,
        _lmsStabilized = null,
