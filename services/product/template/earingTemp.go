...
func RenderEaringTemplate(id int, file_uri string, title string, uid uuid.UUID, organization_id int, width int, length int) string {
	// Parse the template
	tmp, _ := template.New("html").Parse(earingTemplate)
...
func uglifyJs(jsCode []byte) ([]byte, error) {
	cmd := exec.Command("uglifyjs", "-m")
	cmd.Stdin = bytes.NewReader(jsCode)
...
// Define your template here, no need to write it again in the response
const earingTemplate = `<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8" />
	...
	<script type="text/javascript">
		...
		function draw_earrings(landmarks, faceWidth, ry, length, height) {
			// right earring:
			const rightEarringAngleHide =
				-_earringSettings.angleHide -
				_earringSettings.angleHysteresis *
					(_earringsVisibility.right ? 1 : -1);
			if (ry > rightEarringAngleHide) {
				...
				draw_earring(pos, length, height);
				_earringsVisibility.right = true;
			} else {
		...
		function draw_earring(pos, width, height) {
