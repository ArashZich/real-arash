...
func (r *resource) organizationProduct(ctx echo.Context) error {
	// تغییر نوع params
	var params = &endpoints.OrganizationProductQueryRequestParams{}
	...
	count, err := r.service.CountOrganizationProduct(ctx.Request().Context(), *params)
	if err.StatusCode != 0 {
		return ctx.JSON(err.StatusCode, err)
	}
	pages := pagination.NewFromRequest(ctx.Request(), int(count))

	products, er := r.service.OrganizationProduct(
		ctx.Request().Context(),
