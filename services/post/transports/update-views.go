...
func (r *resource) updateViews(ctx echo.Context) error {
	id := ctx.Param("id")
	var input endpoints.UpdateViewsRequest
	...
	post, err := r.service.UpdateViews(ctx.Request().Context(), id, input)
	if err.StatusCode != 0 {
