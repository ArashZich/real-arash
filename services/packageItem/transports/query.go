...
func (r *resource) query(ctx echo.Context) error {
	var params = &endpoints.PackageItemQueryRequestParams{}
	...
	count, err := r.packageItem.Count(ctx.Request().Context(), *params)
	if err.StatusCode != 0 {
		return ctx.JSON(err.StatusCode, err)
	}
	pages := pagination.NewFromRequest(ctx.Request(), int(count))

	packageItems, er := r.packageItem.Query(
		ctx.Request().Context(),
